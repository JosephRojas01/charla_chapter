resource "aws_dynamodb_table" "dynamo_table" {
  ...
  name                        = join("-", tolist([var.client, var.functionality, var.environment, "dynamodb", var.dynamo_config[count.index].application, count.index + 1]))
  ...
  tags = merge({ Name = "${join("-", tolist([var.client, var.functionality, var.environment, "dynamodb", var.dynamo_config[count.index].application, count.index + 1]))}" },
  { id_case = var.dynamo_config[count.index].ticket })
}
